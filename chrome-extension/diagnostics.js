// diagnostics.js — Diagnostic dashboard for RDLSIS Bot v2
document.addEventListener('DOMContentLoaded', () => {
  ...
  function renderTaskList() {
    ...
    if (tasks.length === 0) {
      taskList.innerHTML = '<div class="empty-state">Henüz task kaydı yok. Dashboard\'dan senkronizasyon başlatın.</div>';
      return;
    }
    ...
    taskList.innerHTML = tasks.map(task => {
      ...
      return `
        <div class="task-item ${isActive ? 'active' : ''}" data-task-id="${task.taskId}">
          ...
          <span class="task-external-id" title="${escHtml(task.externalId || '')}">${escHtml(task.externalId || '—')}</span>
          <span class="task-articles-count">${articleCount} makale</span>
    ...
    taskList.querySelectorAll('.task-item').forEach(el => {
      el.addEventListener('click', () => {
        ...
        if (window.innerWidth <= 900) {
          taskDetailSection.classList.add('visible');
          taskDetailSection.scrollIntoView({ behavior: 'smooth' });
        }
      });
    });
  }
  ...
  function renderTaskDetail() {
    ...
    if (task.metrics) {
      ...
      metricsHtml = `
        <div class="metrics-row">
          ...
          <div class="metrics-item"><span class="value">${m.sumOfTimesCited || 0}</span><span class="label">Atıf</span></div>
          <div class="metrics-item"><span class="value">${m.publications || 0}</span><span class="label">Doküman</span></div>
          ${m.i10Index ? `<div class="metrics-item"><span class="value">${m.i10Index}</span><span class="label">i10-index</span></div>` : ''}
          ${m.citingArticles ? `<div class="metrics-item"><span class="value">${m.citingArticles}</span><span class="label">Atıf Yapan</span></div>` : ''}
        </div>
      `;
    }
    ...
    if (articles.length === 0) {
      articlesTbody.innerHTML = '<tr><td colspan="7" class="empty-state">Filtrelere uygun makale bulunamadı.</td></tr>';
      return;
    }
  ...
  function renderPlumx() {
    ...
    if (results.length === 0) {
      plumxTbody.innerHTML = '<tr><td colspan="6" class="empty-state">Henüz PlumX sonucu yok.</td></tr>';
      return;
    }
    ...
    plumxTbody.innerHTML = results.map((r, i) => `
      <tr>
        <td class="col-num">${i + 1}</td>
        <td><span class="plumx-doi" title="${escHtml(r.doi)}">${escHtml(r.doi)}</span></td>
  ...
  document.getElementById('btn-export').addEventListener('click', () => {
    ...
    a.href = url;
    a.download = `rdlsis_diagnostics_${new Date().toISOString().replace(/[:.]/g, '-')}.json`;
    a.click();
    URL.revokeObjectURL(url);
  });

  document.getElementById('btn-clear').addEventListener('click', () => {
    if (!confirm('Tüm diagnostics geçmişini silmek istediğinize emin misiniz?')) return;
